...
void Cube::ParseCubes(const std::wstring& fileName, std::vector<Cube>& cubes)
{
	if (std::fstream is{ fileName })
	{
		...
		for (Value::ConstValueIterator it{ sceneDoc.Begin() }; it != sceneDoc.End(); ++it)
		{
			...
			const Value& type{ jsonCube["layer"] };
			...
			const Value& isOpaque{ jsonCube["opaque"] };
			...
			const Value& positions{ jsonCube["positions"] };
			...
			for (Value::ConstValueIterator locationIt{ positions.Begin() }; locationIt != positions.End(); ++locationIt)
			{
				const Value& location = *locationIt;
				...
				const Value& x = location[0];
				const Value& y = location[1];
				const Value& z = location[2];
